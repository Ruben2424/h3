...
pub trait Connection<B: Buf>: OpenStreams<B> {
    ...
    /// Polls the Connection for Incoming Streams
    ///
    /// Returning ['IncomingStreamType'] for the next incoming stream.
    /// Returning `Err` implies the connection is closing or closed.
    fn poll_incoming(
        &mut self,
